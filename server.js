...
// -------------------- STATIC ROUTES --------------------
app.use("/storage", express.static(STORAGE_BASE, { fallthrough: true }));
...
// --- Complete (sign) envelope ---
app.post(
  "/api/envelopes/:token/complete",
  verifyJWT,
  uploadSignedFile.single("file"),
  async (req, res) => {
    try {
      const env = await Envelope.findById(req._id);
      if (!env) return res.status(404).json({ error: "Envelope not found" });
